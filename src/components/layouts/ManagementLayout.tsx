...
export default function ManagementLayout({
  ...
  const isAdminRouteActive = (currentPath: string, routePath: string) => {
    const normalize = (path: string) => path.replace(/\/+$/, '');

    const current = normalize(currentPath);
    let route = normalize(routePath);

    // 👇 CHỈ xử lý khi có /all
  ...
  const checkActive = isAdminDashboard
    ? isAdminRouteActive
    : isRouteActive;
            ...
            {sidebarConfig.tabs && (
              <div className="space-y-0.5">
                {sidebarConfig.tabs.map((tab) => {
                  const isActive = checkActive(pathname, tab.href);
